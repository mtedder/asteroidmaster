init = ->
  ...
  # sphere - earth
  earth = new THREE.Mesh(new THREE.SphereGeometry(200, 50, 50), new THREE.MeshLambertMaterial(color: 0x0000ff))
  earth.position.z = 0
  ...
  # sphere - moon
  moon = new THREE.Mesh(new THREE.SphereGeometry(200, 50, 50), new THREE.MeshLambertMaterial(color: 0x000000))
  ...
  # sphere - asteroid
  asteroid = new THREE.Mesh(new THREE.SphereGeometry(200, 50, 50), new THREE.MeshLambertMaterial(color: 0x000000))
  ...
  # sphere - space probe1
  probe1 = new THREE.Mesh(new THREE.SphereGeometry(200, 50, 50), new THREE.MeshLambertMaterial(color: 0x000000))
  
  #probe1 orbit calculation (GM = 1)
  r = 6628140 #radius
  v = 4000 #launch velocity
  zenithangle = .7853 #1.5707;//1.5533;
  gmsun = 3.986005e14
  orbitscale = .001
  probe1a = 1 / ((2 / r) - ((v * v) / gmsun)) #semi-major axis
  gutse = Math.pow((r * v * v) / gmsun - 1, 2) * Math.pow(Math.sin(zenithangle), 2) + Math.pow(Math.cos(zenithangle), 2)
  probe1e = Math.sqrt(gutse) #eccentricity
  probe1b = probe1a * Math.sqrt(1 - Math.pow(probe1e, 2))
  q = 0 #45 * .017;//angle between the X-axis and the major axis of the ellipse in radians
  #alert(probe1b);
...
#
updateOrbits = ->
  a = 3000 #asteroid orbit semi-major axis
  b = 2000 #asteroid orbit semi-minor axis
  q = 45 * .017 #angle between the X-axis and the major axis of the ellipse in radians
  ...
  rotation = angle * .017
  moonangle = rotation * 2.0
  asteroidangle = rotation
...
camera = undefined
scene = undefined
miniRenderer = undefined
miniViewCamera = undefined
sphere = undefined
